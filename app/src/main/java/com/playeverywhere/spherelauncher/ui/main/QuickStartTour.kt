...
@Composable
fun QuickStartTour(onComplete: () -> Unit) {
    val context = LocalContext.current
    val lifecycleOwner = LocalLifecycleOwner.current
    var setupPage by rememberSaveable { mutableStateOf(false) }
    var isHome by remember { mutableStateOf(isDefaultLauncher(context)) }

    DisposableEffect(lifecycleOwner, context) {
        val observer = LifecycleEventObserver { _, event ->
            if (event == Lifecycle.Event.ON_RESUME) isHome = isDefaultLauncher(context)
        }
        ...
        text = {
            Column(
                ...
                if (setupPage) {
                    ...
                    if (!isHome) {
                        Button(onClick = { openDefaultLauncherSettings(context) }, modifier = Modifier.fillMaxWidth()) {
